...
def doc_to_synsets(doc):
    ...
    # if there is nothing in the synset for the token, skip it, therefore check len of the synset is > 0
    # return a list of lists of synsets - one list for each token!
    # use only the first match for each token wn.synsets(x,y)[0]!
...
def most_similar_docs():
    ...
    sim_scores = [(x, y, document_path_similarity(x,y)) for x,y in zip(paraphrases['D1'], paraphrases['D2'])]
    
    similarity = max(sim_scores, key=lambda item:item[2])
...
def topic_names():
    ...
    # Your Code Here
    topics= ['Health', 'Science', 'Automobiles', 'Government', 'Computers & IT', 'Sports', 'Business', 'Society & Lifestyle', 'Religion', 'Education']
