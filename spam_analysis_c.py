...
def answer_two():
     ...
     #tokens and their counts as a dictionary:
    vocabulary = CountVectorizer().fit(X_train).vocabulary_

    vocabulary = [x for x in vocabulary.keys()]
    
    #lengths in a separate list:
    len_vocab = [len(x) for x in vocabulary]
    
    # index of the longest token:
    return vocabulary[np.argmax(len_vocab)]
...
def answer_four():
    ...
    feature_names = vectorizer.get_feature_names() # features names
    idfs = vectorizer.idf_ # their value
